...
These four interfaces are the **agreed boundary** every workstream builds against. They were transcribed from `docs/internal/CONTRACTS.md`. Import these from minute one — don't wait for implementations.
...
- Signatures are intentionally close to `docs/internal/CONTRACTS.md`; that doc remains the source of truth for *behavior* (state, math, access control, the CRE on/off-chain boundary).
