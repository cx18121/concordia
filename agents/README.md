...
The 6 demo agents (workstream E in `../docs/internal/ROADMAP.md`): momentum, value, mean-reversion, sector-rotation, low-vol, contrarian. Each is a **deterministic strategy** (reliable, reproducible) + an **LLM thesis layer** that writes the human-readable rationale (outputs are cached — the app must never wait on an LLM API).
