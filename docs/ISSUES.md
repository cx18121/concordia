...
**#C1 — Governance must expose vote-readback views** · OPEN (blocks C↔A integration) · owner: ____ (workstream A).
CRE resolves by recomputing each member's vote-weighted excess from their on-chain allocations, but the frozen `IGovernance` exposes no way to read them. A needs two cheap views over existing storage: `getVoters() returns (address[])` and `allocOf(address) returns (Alloc[])`. The keeper already codes against them (`keeper/src/chain/abi.ts`). (Same story for re-peg: the keeper re-pegs a configured pool list, so it does *not* need a `Vault.heldAssets()` view — avoided that dependency.)

**#C2 — CRE write path needs an `onReport` receiver** · OPEN (CRE-only) · owner: ____ (workstream A).
CRE writes via Chainlink's KeystoneForwarder → `receiver.onReport(bytes,bytes)`, not direct calls. The frozen lifecycle fns are direct `onlyKeeper`. **Recommended: one small `KeeperReceiver` contract** that holds the `onReport`, decodes the payload, and calls the *existing, frozen* `onlyKeeper` fns on Oracle/Governance/Executor (the keeper address just becomes the receiver). Then the 3 contracts barely change. Wire format is single-sourced in `keeper/src/core/encode.ts`: prices `(bytes32[],uint256[],uint256)`, lifecycle `(uint8 action, bytes)` with OPEN=0/LOCK=1/RESOLVE=2, resolve inner `(address[],int256[],uint256[])`, repeg `(bytes32,uint256)`. (Per-contract `onReport` also works — A's call.) **The heartbeat needs none of this** — it calls the frozen `onlyKeeper` fns directly, so the demo still runs if the receiver isn't ready. The receiver is only for the Chainlink-prize CRE path. Note: `onlyKeeper` should accept whichever single driver is active (the receiver *or* the heartbeat EOA), since they're never both live.
