...
**#5a — CRE CLI now requires login even for `simulate`** · OPEN (med) · owner: ____ · found 6/13 building workstream C.
CRE CLI **v1.20.0** gates `cre workflow simulate` behind `cre login` (or `CRE_API_KEY`) — the older docs/template assumption that "simulation needs no access, secrets in local `.env`" is **stale**. What this means: building, testing, and **compiling** the workflow to WASM all work offline (verified: `cre workflow build` produces `binary.wasm` with no auth), but *running* a simulation needs a CRE account. So #5's "request access early" matters more than thought — get one teammate logged in. The CLI install itself needs no auth. Likely still satisfies the prize (sim is runnable once logged in), just front-loads the account step.
...
CRE writes via Chainlink's KeystoneForwarder → `receiver.onReport(bytes,bytes)`, not direct calls. The frozen lifecycle fns are direct `onlyKeeper`. For the CRE workflow to write, Oracle/Governance/Executor each need a small `onReport` that decodes the payload and dispatches (wire format single-sourced in `keeper/src/core/encode.ts`: prices, lifecycle `uint8 action`, resolve, repeg). **The always-on heartbeat (`scripts/run.ts`) needs none of this** — it calls the frozen `onlyKeeper` fns directly via viem, so the human-demo path works without the receiver. The receiver is only for the Chainlink-prize CRE path.
