...
- **KYCHook (`beforeSwap`):** allowlist gate. Since *only the Vault swaps* (members vote, the fund trades collectively), the hook allowlists the **Vault**. Story: "tokenized-stock pools enforce on-chain compliance — only a verified, KYC'd fund can trade them," and the Vault is verified because every depositor passed World ID. Deploy needs CREATE2 address-mining (`HookMiner` from v4-periphery) so the hook address encodes the `beforeSwap` flag.
- **Swaps:** `executeBasket` / `closePositions` route through Universal Router (`V4_SWAP` → `SWAP_EXACT_IN_SINGLE` + `SETTLE_ALL` + `TAKE_ALL`), Permit2-approved.
